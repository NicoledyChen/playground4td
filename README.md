...
### Auto Merge + Push to Hugging Face (Background Queue)

This repo supports **auto-merging** training checkpoints (FSDP shards) into HuggingFace format, **pushing to Hugging Face Hub**,
and **deleting raw shard/optimizer files** to save disk.

Key properties:
- Training process only **enqueues a tag** (e.g. `global_step_123`) into an on-disk queue.
- A separate worker process performs merge/upload/cleanup in the background.
- The **latest checkpoint** is always kept in raw format for resume training.

#### 1) Enable in config

Add to your config file (under `trainer:`):

```yaml
  auto_merge:
    enabled: true
    # Upload target on Hugging Face Hub:
    # - Single repo mode (default): upload into subfolders inside ONE repo.
    # - Per-step repo mode: upload each checkpoint into its OWN repo named by step index.
    hf_repo_id: your_org_or_user/qwen2.5-7b-instruct   # used as prefix when `repo_per_step: true`
    repo_per_step: true
    hf_repo_id_per_step_template: "{hf_repo_id}-step-{step}"  # e.g. qwen2.5-7b-instruct-step-1
    hf_path_in_repo_template: null                           # upload to repo root (recommended for per-step repos)
    update_latest: false                                     # optional
    merge_components: ["actor"]                   # usually only actor is needed
    keep_last_n_raw: 1                            # keep newest raw checkpoint for resume
    keep_best_raw: true
    delete_raw: true
```
...
```bash
python3 scripts/merge_worker.py --save_checkpoint_path checkpoints/easy_r1/your_experiment_name
```

The worker will read tasks from `checkpoints/.../.merge_queue/` and update per-task JSON progress under:
