...
class RayPPOTrainer:
    ...
    def _save_checkpoint(self) -> None:
        ...
        # NOTE:
        # - When auto-merge is enabled, we DO NOT delete checkpoints in the training process.
        #   Old checkpoints will be cleaned up by the background merge worker after merge/upload,
        #   while the latest raw checkpoint is kept for resume.
        if not getattr(self.config.trainer.auto_merge, "enabled", False):
        ...
        # Enqueue background merge task (rank-0 / driver only).
        if getattr(self.config.trainer.auto_merge, "enabled", False):
            try:
                from ..utils.checkpoint.auto_merge_queue import enqueue_tag, write_queue_config

                queue_dir = self.config.trainer.auto_merge.queue_dir
                # Persist config for worker convenience.
                write_queue_config(
                    queue_dir,
                    {
                        "save_checkpoint_path": os.path.abspath(self.config.trainer.save_checkpoint_path),
                        **self.config.trainer.auto_merge.__dict__,
                    },
                )
                tag = f"global_step_{self.global_step}"
                enqueue_tag(queue_dir, tag)
                print(f"[auto-merge] Enqueued merge task: {tag} (queue_dir={queue_dir})")
            except Exception as e:
                # Never fail training due to background merge queue issues.
                print(f"[auto-merge] Failed to enqueue merge task: {e}")
